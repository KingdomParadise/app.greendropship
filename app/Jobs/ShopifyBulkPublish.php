            ...
            do {
                ...
                if ($prod == null) {
                    ...
                    if ((int)$response_product['result'] == 1) {
                        ...
                        $myProducts->save();
                        $collection = json_decode($product)->collections;
                        if ($collection) {
                            $collection_db = DB::table('shopify_collections')->where([['user_id', Auth::User()->id], ['title', $collection]])->first();
                            if ($collection_db) {
                                $collectionId = $collection_db->id;
                            } else {
                                $collections = ShopifyAdminApi::getCollections($this->user);
                                $filtered = $collections->first(function ($value) use ($collection) {
                                    return trim($collection) != '' && $value['name'] == trim($collection);
                                });
                                
                                if ($filtered == null) { //no existe
                                    $collectionId = ShopifyAdminApi::createCustomCollection($this->user, $collection);
                                } else {
