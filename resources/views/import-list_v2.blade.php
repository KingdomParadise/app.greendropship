        ...
        <div class="wrapinsidecontent">
            ...
            <div class="empty-product" style="display: none;">
                <div class="d-flex flex-column align-items-center">
                    ...
                    <h4 style="line-height: 1.5;" class="my-0">Go to Search Products and start adding products to your import list.</h4>
                    <h4 style="line-height: 1.5;" class="my-0">When your import list is ready, you can add the products to your store.</h4>
...
<script type="text/javascript">
                ...
                <div class="producttabs">
                    ...
                    <div class="contenttabs">
                        <div class="tab-1 wpadding tabcontent active">
                            <h3 class="py-2 productname">${res.name}</h3>
                                ...
                                <div>
                                    <div class="editform">
                                        ...
                                        <div class="full">
                                            ...
                                            <div id="tags">
                                                <input type="text" list="tag${res.id_import_list}" id="tags${res.id_import_list}" data-id="${res.id_import_list}">
    ...
    $('#import-products').on('keyup', '.collection', function(e) {
    ...
    let type_timer;
    $('#import-products').on('keyup', '.type', function(e) {
    ...
    let tag_timer;
    $('#import-products').on('keyup', '#tags input', function(e) {
        clearTimeout(tag_timer);
        tag_timer = setTimeout(() => {
        ...
        if (/(188|13)/.test(e.which)) {
            let txt = this.value.replace(/[^a-zA-Z0-9\+\-\.\#]/g, ''); // allowed characters list
            if (txt) $(this).before('<span class="tag">' + txt + '</span>');
            this.value = "";
        }
    });

    $('#import-products').on('change', '.defaulttags input', function(e) {
        let txt = e.target.nextElementSibling.innerText;
        let tag_input = e.target.parentElement.nextElementSibling;
        if (e.target.checked) {
            if (txt) {
                let new_tag = document.createElement('span');
                new_tag.className = 'tag';
                new_tag.innerText = txt;
                tag_input.children[tag_input.children.length - 2].before(new_tag);
            }
        } else {
            for (let i = 0; i < tag_input.children.length; i++) {
                if (tag_input.children[i].innerText == txt) {
                    tag_input.children[i].remove();
                }
            }
        }
    });

    $('#import-products').on('keydown', '#tags input', function(e) {
        clearTimeout(tag_timer);
    });

    $('#import-products').on('click', '#tags .tag', function(e) {
        let default_tags = $(this).parent()[0].previousElementSibling.children;
        for (let i = 0; i < default_tags.length; i++) {
            if (default_tags[i].innerText == $(this)[0].innerText) {
                default_tags[i - 1].checked = false;
            }
        }
        $(this).remove();
    });
